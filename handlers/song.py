...
def bul(client, message):
    ...
    try:
        ...
        m.edit(
            "❌ Heçnə tapa biləmədim. Bağışlayın.\n\nBaşqa bir mahnı yoxlayın vəya mahnı adını düzgün yazln."
        )
        print(str(e))
        return
    m.edit("`Mahnı Yüklənir, Xaiş gözləyin ...⏱`")
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info_dict = ydl.extract_info(link, download=False)
            audio_file = ydl.prepare_filename(info_dict)
            ydl.process_info(info_dict)
        rep = f"⚡️ **ɴᴀᴍᴇ**: [{title[:35]}]({link})\n🇦🇿 **Mənbə**: YouTube\n⏱️ **Vaxt**: `{duration}`\n👁‍🗨 **İZLƏNMƏ**: `{views}`\n📤 **TƏRƏfİndan**: **YOUTUBE**"
